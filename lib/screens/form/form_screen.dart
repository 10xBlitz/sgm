...
class _FormScreenState extends State<FormScreen> {
  ...
  Future<void> _loadQuestions() async {
    ...
    try {
      final questions = await FormQuestionService().fetchQuestionsByForm(
        widget.formId,
      );
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      final status = await ProjectTaskStatusService().getOrCreateNewStatus(
        widget.projectId,
      );
      ...
      final task = await TaskService().createTask(
        ...
        customerCountryResidence:
            _textControllers[kCountryResidence]?.text ?? '',
        customerPhone:
            _textControllers[kPhoneNumber]?.text.replaceAll(
              RegExp(r'[^0-9]'),
              '',
            ) ??
            '',
      ...
      if (_questions != null) {
        for (final question in _questions!) {
          ...
          var created = await TaskFormService().createResponse(
            ...
            checkedBox:
                data[TaskFormResponseRow.field.checkedBox] as List<String>?,
            questionText: question.question,
          );
          MyLogger.d(
            'Created response: ${question.question} -- ${data.toString()} ${created?.id}',
          );
        }
      }
      LoadingUtils.dismissLoading();
  ...
  Map<String, dynamic> _buildResponseData(
    FormQuestionRow question,
    String? taskId,
    ...
    switch (question.type) {
      case 'text':
        data[TaskFormResponseRow.field.answer] =
            _textControllers[question.id]?.text;
        break;
      case 'checkbox':
  ...
  Future<String?> uploadFormFileToSupabase(
    PlatformFile file, {
    String? userId,
  }) async {
    try {
      ...
      final fileName =
          '${userId ?? 'anon'}-${DateTime.now().millisecondsSinceEpoch}$fileExt';
